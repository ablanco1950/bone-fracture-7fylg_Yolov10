...
def plot_image(image, boxes, boxesTrue, imageCV):
    ...
    print("BOXES")
    print(boxesTrue)
    if boxesTrue[4]=="" or boxesTrue[2]=="":
         print ("Raro Boxes empty")
         boxesTrue[2]=0
         #boxesTrue[3]=0
         boxesTrue[4]=0
         #boxesTrue[5]=0
    print( float( boxesTrue[2]))
    print( float( boxesTrue[4])) 
    upper_left_x_true =float( boxesTrue[2]) - float(boxesTrue[4]) / 2
    upper_left_y_true = float(boxesTrue[3]) - float(boxesTrue[5]) / 2
    
    print("BOXES")
    print(boxesTrue)
   
    upper_left_x_true =float( boxesTrue[0]) - float(boxesTrue[2]) / 2
    upper_left_y_true = float(boxesTrue[1]) - float(boxesTrue[3]) / 2
    rect1 = patches.Rectangle(
            (upper_left_x_true * width, upper_left_y_true * height),
           float( boxesTrue[2]) * width,
           float( boxesTrue[3]) * height,
            linewidth=2,
            edgecolor="green",
            facecolor="none",
        )
...
for i in range (len(imagesComplete)):
            ...
            plot_image(img, Tabnms_boxes, TabxyxyTrue[i], img)
